...
#' This function is the first of the main functions in this package and is a dumbed down version of norm_tidy_dbf and normfluordbf. It is here as a reminder of where the inventor started.
#' This function needs to be included to ensure the continuum of education and it is the 'ROOT'.
#' normfluordbf_legacy will be equivalent to norm_tidy_dbf_legacy (this does not exist).
#'
#' @examples
#' fpath <- system.file("extdata", "liposomes_214.dbf", package = "normfluodbf", mustWork = TRUE)
#' normalized_dbf <- normfluordbf_legacy(file=fpath)

normfluordbf_legacy <- function(file = NULL, fun = NA, ...){
