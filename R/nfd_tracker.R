...
#'
#' @examples nfd_tracker(period = 'last-month', plot = 'cum')

nfd_tracker <- function(period=NULL, plot = NULL){
  if(period == 'last-month' || plot == 'cum'){
    ...
    cran_downloads(packages="normfluodbf", when="last-week") #last-week, last-month
    nfd_totals <- cran_downloads(packages="normfluodbf", from = "2023-08-25", to = Sys.Date()-1)
    sum(nfd_totals[,2])
