...
```{r include=FALSE}
#tools::compactPDF("inst/extdata", gs_quality = "ebook")
#tools::compactPDF("doc", gs_quality = "ebook") #gs+qpdf
#goodpractice::gp()
...
# inteRgrate::check_news()
# 
# #submit to cran
# devtools::test()
# devtools::test_coverage()
# devtools::run_examples()
# devtools::release() #submit(best part)
```

```{r include=FALSE, eval=FALSE}
library(rio)
data <- import_list("your_file.xlsx")

library(openxlsx)
data <- lapply(getSheetNames("your_file.xlsx"), function(sheet) read.xlsx("your_file.xlsx", sheet = sheet))
names(data) <- getSheetNames("your_file.xlsx")
...
read_all_sheets <- function(filename) {
  ...
  data_list <- lapply(sheets, function(sheet) read_excel(filename, sheet = sheet))
...
gg_rookie <- function(data, x, wells, xlim = NULL, ylim = NULL,
                         labels = wells, dark = TRUE) {
  stopifnot(is.data.frame(data))
  if (!is.character(x) || length(x) != 1) stop("`x` must be a single column name (character).")
  missing <- setdiff(wells, colnames(data))
  if (length(missing)) stop("Missing columns: ", paste(missing, collapse = ", "))
  if (length(labels) != length(wells)) stop("`labels` must be same length as `wells`.")
  ...
  df_long$role <- factor(df_long$well, levels = wells, labels = labels)

  p <- ggplot2::ggplot(df_long, ggplot2::aes(x = .data[[x]], y = fluor, color = role, group = role)) +
    ggplot2::geom_line(size = 0.8) +
    ggplot2::geom_point(size = 1.8, alpha = 0.85) +
    ggplot2::labs(
      title = "NavAb Liposome Flux Assay System: NOISY PLOT",
      subtitle = "Noise emerging from liposome flux assays when reagent concentrations are too high or too low.",
...
```{r qc_demo, warning=FALSE, message=FALSE, comment=FALSE, echo=FALSE, fig.path = "man/figures/", fig.width=12, fig.height=8}
...
xvar  <- "Cycle_Number"
wells <- c("A01", "A02", "A03")
labs  <- c("Test", "Negative Control", "Positive Control")
...
```{r visualization_demo, warning=FALSE, message=FALSE, comment=FALSE, echo=FALSE, fig.path = "man/figures/", fig.width=12, fig.height=8}
...
CRAN](https://www.r-bloggers.com/2020/07/how-to-write-your-own-r-package-and-publish-it-on-cran/)
